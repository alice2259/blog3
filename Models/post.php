    ...
    public static function search() {
      ...
      $db = Db::getInstance();
        ...
        if(isset($_POST['search'])){
            ...
            $sql = "SELECT post.postID, post.title, userTable.userID, post.datePublished, post.headerImage, post.imageCaption, post.content, 
                userTable.profilePic, userTable.firstName, userTable.surname FROM post 
                INNER JOIN userTable ON post.userID=userTable.userID 
                WHERE post.title LIKE '%".$input."%' 
                    UNION 
                    SELECT post.postID, post.title, userTable.userID, post.datePublished, post.headerImage, post.imageCaption, post.content, 
                    userTable.profilePic, userTable.firstName, userTable.surname FROM post 
                    INNER JOIN userTable ON post.userID=userTable.userID 
                    WHERE userTable.firstName LIKE '%".$input."%' 
                        UNION SELECT post.postID, post.title, userTable.userID, post.datePublished, post.headerImage, post.imageCaption, post.content, 
                        userTable.profilePic, userTable.firstName, userTable.surname FROM post 
                        INNER JOIN userTable ON post.userID=userTable.userID 
...
//            . "WHERE userTable.surname LIKE '%".$input."%'"; 
        ...
        $list[] = new Post($post['postID'], $post['title'],$post['userID'], 
                $post['datePublished'], $post['headerImage'], $post['imageCaption'], 
                $post['content'], $post['profilePic'], $post['firstName'], $post['surname']);
